...
def sharpe_ratio(returns, weights = None, risk_free_rate = 0.015,sample_freq='d'):
    ...
    if sample_freq == 'd':
        sharpe_ratio_val = sharpe_ratio_val * np.sqrt(252)
    elif sample_freq == 'w':
        sharpe_ratio_val = sharpe_ratio_val * np.sqrt(52)
    elif sample_freq == 'm':
        sharpe_ratio_val = sharpe_ratio_val * np.sqrt(12)
    else:
...
if __name__ == "__main__":
    ...
    print "Initial Sharpe Ratio ", sharpe_ratio(returns, initial_weights, risk_free_rate)

    optimized_weights = optimize_portfolio(returns, risk_free_rate)
    print "optimized_weights ", optimized_weights
    
    print "Optimized Sharpe Ratio", sharpe_ratio(returns, optimized_weights, risk_free_rate)
